...
SELECT ModifiedDate, CustomerID, SalesPersonID FROM Sales.SalesOrderHeader ORDER BY ModifiedDate DESC;
--3 kolumny tabeli [Person].[Contact], posortowane wg LastName malej¹co i FirstName rosn¹co
SELECT EmailAddress, LastName, FirstName FROM Person.Contact ORDER BY LastName DESC, FirstName ASC;
...
SELECT * FROM HumanResources.Employee WHERE Title LIKE 'Production Technician%' OR Title LIKE 'Tool Designer%' ORDER BY Title;
